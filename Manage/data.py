...
class MSOADataGenerate(Dataset):
    ...
    def _generate_by_msr(self, data):
        ...
        region = GISTRegion if self.dataset == "GIST" else NeuralScanRegion
...
class DataManage:
    ...
    def _gen_features_wavelet(self, x, wavelet='db4', level=4, axis=0):
        """
    ...
    def getData(self):
        ...
        # 其它有数据增强操作的网络需要自定义（DRL, MSOA et al）
        ...
        if self.mode == True:
            ...
            #     # if i != self.sub_id:
            #     if first_flag:
            #         x = np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{i:>02d}', f'x_{field}.npy'))
            #         y = np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{i:>02d}', f'y_{field}.npy'))
            #         first_flag = False
            #     else:
            #         x = np.concatenate((x, np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{i:>02d}', f'x_{field}.npy'))))
            #         y = np.concatenate((y, np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{i:>02d}', f'y_{field}.npy'))))
        else:
            x = np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{self.sub_id:>02d}', f'x_{field}.npy'))
            y = np.load(os.path.join(os.getcwd(), 'Dataset', self.dataset, f'S{self.sub_id:>02d}', f'y_{field}.npy'))
        ...
        # io.savemat("test.mat", {'data': x})

        # exit()
        # x = io.loadmat(os.path.join(os.getcwd(), 'Dataset', "THU_NEST", f'S{self.sub_id:>02d}', f'x_{field}.mat'))
        # x = np.array(x['EEG']['data'][0][0])
        # # io.savemat(os.path.join(os.getcwd(), 'Dataset', "THU_NEST", f'S{self.sub_id:>02d}', f'x_{field}.mat'), {'data': x})
        # x = np.transpose(x, (2, 0, 1))
        # np.save(os.path.join(os.getcwd(), 'Dataset', "THU_NEST", f'S{self.sub_id:>02d}', f'x_{field}.npy'), x)
        # np.save(os.path.join(os.getcwd(), 'Dataset', "THU_NEST", f'S{self.sub_id:>02d}', f'y_{field}.npy'), y)


        # 数据预处理
        # x = self.preprocesser.re_reference_data_LM(x)
        x = self.preprocesser.band_pass_filter(data=x, freq_low=0.1, freq_high=48, fs=256)
        ...
        #     xn.append(self._gen_features_wavelet(x[i, ...].T))
        # x = np.stack(xn)
        # try:
        #     x = self.preprocesser.euclidean_space_alignment(x)
        # except:
        #     print("EA未执行")

        x_npy, y_npy = copy.deepcopy(x), copy.deepcopy(y)

        # （DRL, MSOA et al）带来的附加操作
        if self.method_name == "MSOA":
            data = MSOADataGenerate(self.dataset, x, y, self.sub_id, self.mode)
            data_loader = DataLoader(data, batch_size=self.batch_size, shuffle=(self.mode is True))
        elif self.method_name == "MTCN":
            if self.mode == True:
                data = MSOADataGenerate(self.dataset, x, y, self.sub_id, self.mode)
                data_loader = DataLoader(data, batch_size=self.batch_size, shuffle=(self.mode is True))
            else:
